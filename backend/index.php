<?php
    if ($_SERVER['HTTP_ORIGIN'] === "http://127.0.0.1:1234" || 
        "http://localhost:1234" || "https://no-idea-web.fr" ||
        "http://localhost:4040" )
    {
        ...
        $router->map('POST', '/[*:controller]/[*:fn]', function ($controller, $fn) {
            ...
            {
                $instance = new $cls();
                return \My_class\Config::send_JSON($instance->$fn(json_decode($post)));
            } 
            else require("forbidden.html");
        });

        $router->map('PUT', '/[*:controller]/[*:fn]', function ($controller, $fn) {
            $cls = '\Controller\\' . ucfirst($controller);
            if (class_exists($cls) && method_exists($cls, $fn))
            {
                $instance = new $cls();
        ...
        $router->map('DELETE', '/[*:controller]/[*:fn]', function ($controller, $fn) {
            $cls = '\Controller\\' . ucfirst($controller);
            if (class_exists($cls) && method_exists($cls, $fn))
            {
                $instance = new $cls();
